...
$allowed_sort = ['title', 'category', 'date', 'created_at'];
...
if (!empty($search)) {
    $whereSQL = "WHERE title LIKE ? OR category LIKE ?";
    $searchTerm = "%" . $search . "%";
    $params[] = $searchTerm;
    $params[] = $searchTerm;
    $types .= "ss";
...
$sql = "SELECT * FROM projects $whereSQL ORDER BY $sort $order LIMIT ? OFFSET ?";
...
// Helper to generate sort links (for full loads)
function sortLink($col, $currSort, $currOrder, $search)
{
    $newOrder = ($currSort === $col && $currOrder === 'ASC') ? 'DESC' : 'ASC';
    return "?sort=$col&order=$newOrder&search=" . urlencode($search);
}
...
if (isset($_GET['ajax'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $imgSrc = ($row['image'] && file_exists("../assets/files/projects/" . $row['image']))
                ? "../assets/files/projects/" . htmlspecialchars($row['image'])
                : "../assets/files/people/placeholder.png"; // Placeholder

            echo '<tr>';
            echo '<td>' . $i++ . '</td>';
            echo '<td><img src="' . $imgSrc . '" alt="Project" style="height: 50px; width: 50px; object-fit: cover; border-radius: 4px;"></td>';
            echo '<td>' . htmlspecialchars(mb_strimwidth($row['title'], 0, 50, "...")) . '</td>';

            $badgeColor = match (strtolower($row['category'])) {
                'ongoing' => 'info',
                'completed' => 'success',
                'upcoming' => 'warning',
                default => 'secondary'
            };
            echo '<td><span class="badge bg-' . $badgeColor . ' text-dark">' . ucfirst(htmlspecialchars($row['category'] ?? 'N/A')) . '</span></td>';

            echo '<td>' . htmlspecialchars($row['date'] ?? 'N/A') . '</td>';
            echo '<td class="text-end">
                    <a href="edit-project.php?id=' . $row['id'] . '" class="btn btn-sm btn-outline-primary me-1"><i class="fas fa-edit"></i></a>
                    <a href="delete-project.php?id=' . $row['id'] . '" class="btn btn-sm btn-outline-danger" onclick="return confirm(\'Are you sure you want to delete this project?\');"><i class="fas fa-trash"></i></a>
                  </td>';
            echo '</tr>';
        }
    } else {
...
<head>
    ...
    <title>Manage Projects - IndoNorway Connect</title>
    <!-- Fonts -->
    <link
                        ...
                        <table class="table table-hover table-custom mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th><a href="#" class="sort-link" data-sort="category">Category <span
                                                id="sort-icon-category"><?php echo sortIcon('category', $sort, $order); ?></span></a>
                                    </th>
                                    <th><a href="#" class="sort-link" data-sort="date">Date <span
                                                id="sort-icon-date"><?php echo sortIcon('date', $sort, $order); ?></span></a>
                                    </th>
                                    ...
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                echo match (strtolower($row['category'])) {
                                                    'ongoing' => 'info',
                                                    'completed' => 'success',
                                                    'upcoming' => 'warning',
                                                    default => 'secondary'
                                                ...
                                                ?> text-dark">
                                                    <?php echo ucfirst(htmlspecialchars($row['category'] ?? 'N/A')); ?>
                                                </span>
                                            </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function fetchData(page = 1) {
                ...
                fetch('projects.php?' + params.toString())
                    ...
                    .then(data => {
                        ...
                        // Rebind pagination clicks since HTML replaced
                        bindPaginationLinks();
                    })
            ...
            document.querySelectorAll('.sort-link').forEach(link => {
                link.addEventListener('click', function (e) {
                    ...
                    if (currentSort === col) {
                        ...
                        currentSort = col;
                        currentOrder = 'DESC'; // Default new sort to DESC
                    }
