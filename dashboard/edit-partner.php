...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($mimeType, $allowedTypes)) {
            ...
            // Simple check to avoid overwriting unrelated files (rudimentary)
            // Ideally use unique IDs or timestamps
    ...
    if (!$error) {
        if ($id) {
            // Update
            $stmt = $conn->prepare("UPDATE partners SET title = ?, image = ? WHERE id = ?");
            ...
            // Insert
            $stmt = $conn->prepare("INSERT INTO partners (title, image) VALUES (?, ?)");
...
<head>
    ...
    <title>
        <?php echo $id ? 'Edit' : 'Add'; ?> Partner - IndoNorway Connect
    </title>
    <!-- Fonts -->
    <link
            ...
            <div class="card shadow-sm border-0" style="max-width: 800px;">
                <div class="card-body p-4">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="mb-4">
                            <label for="title" class="form-label fw-bold">Partner Name / Title</label>
                            <input type="text" class="form-control" id="title" name="title"
                                value="<?php echo $id ? htmlspecialchars($partner['title']) : ''; ?>" required>
                        </div>

                        <div class="mb-4">
                            <label for="image" class="form-label fw-bold">Partner Logo</label>
                            <?php if ($id && !empty($partner['image'])): ?>
                                <div class="mb-2 p-3 border rounded bg-light mb-3">
                                    ...
                                    <img src="../assets/files/home/<?php echo htmlspecialchars($partner['image']); ?>"
                                        alt="Current Image" class="img-fluid" style="max-height: 100px;">
                                </div>
                            <?php endif; ?>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            <button type="button" class="btn btn-light"
                                onclick="window.location.href='partners.php'">Cancel</button>
                            <button type="submit" class="btn btn-primary"
                                style="background: var(--bg-dark-blue); border: none;">
                                <?php echo $id ? 'Update Partner' : 'Add Partner'; ?>
                            </button>
                        </div>
                    </form>
                </div>
