...
if ($id) {
    $stmt = $conn->prepare("SELECT * FROM herobanner WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($mimeType, $allowedTypes)) {
            ...
            // Add timestamp to prevent overwrites/caching issues
    ...
    if (!$error) {
        if ($id) {
            // Update
            $stmt = $conn->prepare("UPDATE herobanner SET title = ?, description = ?, image = ? WHERE id = ?");
            ...
            // Insert
            $stmt = $conn->prepare("INSERT INTO herobanner (title, description, image) VALUES (?, ?, ?)");
...
<head>
    ...
    <title>
        <?php echo $id ? 'Edit' : 'Add'; ?> Banner - IndoNorway Connect
    </title>
    <!-- Fonts -->
    <link
            ...
            <div class="card shadow-sm border-0" style="max-width: 800px;">
                <div class="card-body p-4">
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="mb-4">
                            <label for="title" class="form-label fw-bold">Banner Title</label>
                            <input type="text" class="form-control" id="title" name="title"
                                value="<?php echo $id ? htmlspecialchars($banner['title']) : ''; ?>" required>
                        </div>

                        <div class="mb-4">
                            <label for="description" class="form-label fw-bold">Banner Description</label>
                            <textarea class="form-control" id="description" name="description" rows="4"
                                required><?php echo $id ? htmlspecialchars($banner['description']) : ''; ?></textarea>
                        </div>

                        <div class="mb-4">
                            <label for="image" class="form-label fw-bold">Banner Image</label>
                            <?php if ($id && !empty($banner['image'])): ?>
                                <div class="mb-2 p-3 border rounded bg-light mb-3">
                                    <p class="small text-muted mb-2">Current Image:</p>
                                    <img src="../assets/files/home/<?php echo htmlspecialchars($banner['image']); ?>"
                                        alt="Current Image" class="img-fluid" style="max-height: 150px;">
                                </div>
                            <?php endif; ?>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            <button type="button" class="btn btn-light"
                                onclick="window.location.href='banner.php'">Cancel</button>
