...
// Fetch latest 3 projects where title is present
// Image allows null (will use default)
$sql = "SELECT id, title, category, image, overview , Description FROM projects 
    ...
    <div class="activity-cards">
        ...
        if ($result && $result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $overview = htmlspecialchars($row['Description'] ?? '');

                // Image handling
                $imagePath = 'assets/files/projects/placeholder-batteries.webp';
                if (!empty($row['image'])) {
                    $imagePath = 'assets/files/projects/' . htmlspecialchars($row['image']);
                }

                // Dynamic class for cards
                $cardClass = ($count == 1) ? 'activity-card-01' : 'activity-card-' . $count;
                ?>
                <!-- Project Card <?php echo $count; ?> -->
                <div class="<?php echo $cardClass; ?> card-item"
                    onclick="window.location.href='InnerProjects.php?id=<?php echo $id; ?>'" style="cursor: pointer;">
                    <div class="card-image-container">
                        <img src="<?php echo $imagePath; ?>" alt="<?php echo $title; ?>" class="card-01-img"
                            onerror="this.onerror=null;this.src='assets/files/projects/placeholder-batteries.webp';">
            ...
            echo '<p class="text-center text-muted">No projects available at the moment.</p>';
        }
        ?>
    </div>
...
<style>
    ...
    .project-description {
        ...
        display: -webkit-box;
        -webkit-line-clamp: 6;
        /* Maximum total lines (title + description) */
        -webkit-box-orient: vertical;
        overflow: hidden;
        text-overflow: ellipsis;
